...
**228 testes passando** ao final deste fechamento (230 da v0.7.3 −3 testes de papel +1 de gating de usuários). ADRs 0001–0057. Pendência (revisar **após uso real/produção**): o painel inline de Estado da ADR 0044 (`EstadoForm` + view `demanda_estado`) **foi removido** na reescrita visual — não é endpoint órfão a apagar (já saiu). As transições automáticas (signals) + Concluir/Arquivar/Reabrir/Editar seguem cobrindo o fluxo; a única consequência é que o status `aguardando_pessoa` ficou **sem setter na UI** (era manual por design). Decidir entre remover o status ou dar um botão simples só faz sentido depois que o sistema for usado de verdade.
...
- **Blueprint `render.yaml`** cria os três recursos de uma vez: Web Service Starter, PostgreSQL Basic-256mb e disco persistente de 1 GB montado em `/var/data` (~US$ 13,55/mês). Endereço **`mpd-6ecl.onrender.com`** (o subdomínio `mpd` estava tomado), região Virgínia — **no ar desde 2026-08-19**. Runtime Python nativo — **Docker adiado** (DT-022).
